...
#logo
logo = pyfiglet.figlet_format('*      dlthon      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'dlthon':
        await bot.send_message(OWNER_ID, f"""**∘ بوت دلثون لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج البوت بن عبدالله - @Bn_abdallah
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if answer.text == 'نعم':
            ...
            for file in os.listdir():
                if file not in ['run.py', 'dlthonkalb.py', 'dlthon.json', '__pycache__', 'dlthon-telethon-cl.py', 'bot.session']:
                    os.remove(file)
        elif answer.text == 'لا':
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ ძᥣƚɦ᥆ꪀ 𝐁𝐎𝐓 ༻', 'button0')]
        ]
...
@bot.on(events.NewMessage(pattern='/python', from_users=5723475089))
